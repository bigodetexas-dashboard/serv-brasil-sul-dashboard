...
def process_delivery(delivery: dict, repo: DeliveryQueueRepository) -> bool:
    ...
    gamertag = delivery.get("gamertag")
    ...
    try:
        ...
        # Attempt FTP upload
        success = upload_spawn_request(item_code, coordinates, gamertag)
    ...
    while True:
        try:
            ...
            if deliveries:
                ...
                print(
                    f"[WORKER] Batch complete: {success_count} success, {fail_count} failed"
                )
            else:
