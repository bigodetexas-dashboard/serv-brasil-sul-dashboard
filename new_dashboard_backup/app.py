...
ALLOWED_ORIGINS = os.getenv("ALLOWED_ORIGINS", "http://127.0.0.1:5000,http://localhost:5000")
if os.getenv("FLASK_ENV") == "production":
    # Em produção, deve ser a URL do Render/Domínio Próprio
    ALLOWED_ORIGINS = os.getenv("PRODUCTION_URL", "https://serv-brasil-sul-dashboard.onrender.com")

# Inicializar SocketIO com CORS restrito
socketio = SocketIO(app, cors_allowed_origins=ALLOWED_ORIGINS.split(","), async_mode="threading")
    ...
    try:
        ...
        if conn_audit:
            try:
                ...
                cur_audit.execute(
                    "INSERT OR REPLACE INTO player_identities (gamertag, xbox_id, last_ip, last_seen) VALUES (?, ?, ?, datetime('now'))",
                    (
...
def api_settings_get():
    ...
    settings = {
        ...
        "display_name": stats.get("nitrado_gamertag") or session.get("discord_username"),
        "username": stats.get("discord_username") or session.get("discord_username", ""),
...
def api_clan_chat_send():
    ...
    success = repo.send_message(clan["id"], user_id, sender_name, content)
...
def api_bank_transfer():
    ...
    if success:
        return jsonify(
            {"success": True, "message": message, "new_balance": repo.get_balance(user_id)}
        )
    else:
        ...
        for item in items:
            ...
            try:
                delivery_id = delivery_repo.add_to_queue(
                    ...
                    purchase_id=None,  # Could link to transaction if we had the ID
                    priority=0,
                )
