...
def create_tile(z, x, y):
    ...
    for city in CITIES:
        ...
        if (
            tile_map_x <= city_x_in_map < tile_map_x + pixels_per_tile_in_map
            and tile_map_y <= city_z_in_map < tile_map_y + pixels_per_tile_in_map
        ):

            # Posição relativa dentro do tile
            rel_x = int(
                ((city_x_in_map - tile_map_x) / pixels_per_tile_in_map) * TILE_SIZE
            )
            rel_y = int(
                ((city_z_in_map - tile_map_y) / pixels_per_tile_in_map) * TILE_SIZE
            )
