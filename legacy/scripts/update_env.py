...
def update_env_file():
    ...
    with open(".env", "w", encoding="utf-8") as f:
        ...
        f.write("# ===== DISCORD BOT =====\n")
        f.write(
            f"DISCORD_TOKEN={final_config.get('DISCORD_TOKEN', 'seu_token_do_bot_aqui')}\n"
        )
        f.write(
            f"ADMIN_PASSWORD={final_config.get('ADMIN_PASSWORD', 'sua_senha_admin_segura')}\n\n"
        )
        ...
        f.write(
            f"NOTIFICATION_WEBHOOK_URL={final_config.get('NOTIFICATION_WEBHOOK_URL', 'seu_webhook_url_aqui')}\n\n"
        )
        ...
        f.write("# ===== NITRADO API =====\n")
        f.write(
            f"NITRADO_TOKEN={final_config.get('NITRADO_TOKEN', 'seu_token_nitrado_api')}\n"
        )
        ...
        f.write("# ===== SEGURANCA =====\n")
        f.write(
            f"ADMIN_WHITELIST={final_config.get('ADMIN_WHITELIST', '123456789,987654321')}\n"
        )
        f.write(
            f"RATE_LIMIT_ENABLED={final_config.get('RATE_LIMIT_ENABLED', 'true')}\n\n"
        )
