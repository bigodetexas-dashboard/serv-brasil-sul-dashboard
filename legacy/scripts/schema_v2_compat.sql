...
DO $$ BEGIN -- Adicionar colunas de customizacao se faltarem
IF NOT EXISTS (
    SELECT 1
...
ALTER TABLE clans
ADD COLUMN color1 VARCHAR(7) DEFAULT '#FF0000';
END IF;
-- Cor 2
IF NOT EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_name = 'clans'
        AND column_name = 'color2'
) THEN
ALTER TABLE clans
ADD COLUMN color2 VARCHAR(7) DEFAULT '#000000';
END IF;
END $$;
...
AND NOT EXISTS (
    ...
    LIMIT 1
) THEN -- Tenta copiar dados da estrutura antiga se compativel
NULL;
-- Por enquanto nao vamos forcar migracao automatica para nao quebrar
END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS bases_v2 (
    id SERIAL PRIMARY KEY,
    owner_discord_id TEXT UNIQUE NOT NULL,
    -- Dono da base (Discord ID)
    clan_id INTEGER,
    -- Opcional: Base de cla
...
CREATE TABLE IF NOT EXISTS nitrado_links (
    ...
    -- Codigo temporario para verificar no chat
    linked_at TIMESTAMP,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
