...
keep_files = {
    ...
    "HOSTING_GUIDE.md",
    "DEPLOY.md",
    "SETUP_GUIDE.md",  # Keep some useful docs
}
...
for item in os.listdir(root):
    ...
    # Skip directories we want to keep or legacy itself
    if os.path.isdir(item_path):
        if item in keep_dirs:
            continue
        # If it's another archive directory, move it to legacy
        if item.endswith("_archive"):
            print(f"Moving archive folder: {item}")
            shutil.move(item_path, os.path.join(legacy_dir, item))
            files_moved += 1
            continue
        # Otherwise it's some other non-essential dir?
        # Let's be careful with dirs.
        continue
    ...
    if item.endswith(".md"):
        ...
        target = legacy_scripts  # or another folder
