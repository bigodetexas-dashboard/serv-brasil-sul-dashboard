...
def get_file_hash(filepath):
    ...
    # usedforsecurity=False tells the audit tools this isn't for cryptographic security
    try:
    ...
    for pattern in important_files:
        if "*" in pattern:
            ...
            if os.path.exists(base_dir):
                for file in os.listdir(base_dir):
                    if file.endswith(ext.replace("*", "")):
                        ...
                        if (
                            src not in index["file_hashes"]
                            or index["file_hashes"][src] != file_hash
                        ):
            ...
            if os.path.exists(pattern):
                ...
                if (
                    pattern not in index["file_hashes"]
                    or index["file_hashes"][pattern] != file_hash
                ):
