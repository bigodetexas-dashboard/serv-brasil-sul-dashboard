...
if players:
    for i, (gamertag, data) in enumerate(players.items(), 1):
        print(f"{i}. {gamertag}")
        print(
            f"   Kills: {data.get('kills', 0)} | Deaths: {data.get('deaths', 0)} | K/D: {data.get('kills', 0) / max(data.get('deaths', 1), 1):.2f}"
        )
        print(
            f"   Best Killstreak: {data.get('best_killstreak', 0)} | Longest Shot: {data.get('longest_shot', 0)}m"
        )
...
if clans:
    for i, (clan_name, data) in enumerate(clans.items(), 1):
        ...
        print(
            f"   Total Kills: {data.get('total_kills', 0)} | Total Deaths: {data.get('total_deaths', 0)}"
        )
        print()
else:
