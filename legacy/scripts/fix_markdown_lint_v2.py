        ...
        if line.strip() == "```":
            ...
            if i + 1 < len(lines):
                ...
                if next_line.startswith("{") or next_line.startswith("["):
                    ...
                    cmd in next_line
                    for cmd in ["npm ", "python ", "cd ", "git ", "pip ", "node "]
                ):
                    fixed_lines.append("```bash")
                elif "=" in next_line and next_line.isupper():
                    fixed_lines.append("```env")
                elif next_line.startswith("http://") or next_line.startswith(
                    "https://"
                ):
                    fixed_lines.append("```text")
                elif any(
                    word in next_line for word in ["PORT", "DATABASE", "SECRET", "API"]
                ):
                    fixed_lines.append("```env")
                else:
