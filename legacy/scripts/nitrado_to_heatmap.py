...
NITRADO_LOG_PATH = (
    "/games/ni123456_1/noftp/dayzxb/config-1/profiles/"  # Ajustar conforme seu servidor
)
...
def send_to_api(log_text):
    ...
    try:
        response = requests.post(
            API_URL, json={"text": log_text, "source": "nitrado_ftp"}, timeout=30
        )

        if response.status_code == 200:
            result = response.json()
            print(
                f"✅ Sucesso: {result['events_saved']} eventos salvos de {result['events_parsed']} parseados"
            )
            return True
        else:
    ...
    while True:
        try:
            ...
            if full_content:
                ...
                if new_content:
                    print(
                        f"📝 {len(new_content.splitlines())} novas linhas encontradas"
                    )
                    # Enviar para API
