...
async function loadUserProfile() {
    try {
        ...
        if (userGamertag) userGamertag.textContent = data.gamertag || 'Não vinculado';
        if (userBalance) userBalance.textContent = formatNumber(data.balance || 0);

        // Xbox Verification Badge
        const xboxBadge = document.getElementById('xbox-verified-badge');
        if (xboxBadge) {
            xboxBadge.style.display = data.xbox_connected_to_discord ? 'inline-block' : 'none';
        }
...
async function loadUserStats() {
    try {
        ...
        if (statKD) statKD.textContent = data.kd || calculateKD(data.kills, data.deaths);
