...
function initMap() {
    console.log('🗺️ Inicializando mapa Chernarus (Tiles HD)...');
    ...
    // ✅ ATUALIZADO: Usar sistema de tiles de alta definição
    // Tiles seguem o padrão Leaflet: /tiles/{z}/{x}/{y}.png
    ...
    L.tileLayer('/static/tiles/{z}/{x}/{y}.png', {
        ...
        noWrap: true,
        bounds: bounds,
        errorTileUrl: '/static/img/chernarus_map.jpg' // Fallback para imagem completa
    }).addTo(map);
...
function setupControls() {
    ...
    document.getElementById('weaponFilter').addEventListener('change', (e) => {
        ...
        // ✅ IMPLEMENTADO: Recarregar dados com filtro de arma
        loadAllData(currentFilters.range);
    });
}

function loadAllData(range) {
    loadHeroStats(range);  // ✅ NOVO: Carregar hero stats
...
async function loadHeroStats(range) {
    try {
        ...
        if (data.success) {
            // Atualizar os 6 cards do hero banner
            document.getElementById('heroTotalKills').innerText = data.total_kills.toLocaleString();
            document.getElementById('heroKillsToday').innerText = data.kills_today.toLocaleString();
            document.getElementById('heroTopWeapon').innerText = data.top_weapon;
            document.getElementById('heroTopPlayer').innerText = data.top_player;
            document.getElementById('heroPeakHour').innerText = data.peak_hour;
            document.getElementById('heroHottestZone').innerText = data.hottest_zone;
        }
    } catch (error) {
...
async function loadHeatmapData(range) {
    try {
        // ✅ NOVO: Incluir filtro de arma na URL
        const response = await fetch(`/api/heatmap?range=${range}&grid=100&weapon=${currentFilters.weapon}`);
...
function updateTopLocationsUI(locations) {
    ...
    locations.forEach((loc, index) => {
        ...
        card.className = 'location-card fade-in';  // ✅ NOVO: Classe fade-in
        card.innerHTML = `
        ...
        card.addEventListener('click', () => {
            ...
            map.setView([lat, lng], 5);  // ✅ MELHORADO: Zoom mais próximo
            // ✅ NOVO: Smooth scroll para o mapa
