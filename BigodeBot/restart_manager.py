            ...
            async with session.post(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    if data.get("status") == "success":
                        print(
                            f"[{datetime.datetime.now()}] ✅ SUCESSO: Servidor reiniciando!"
                        )
                        return True
                    else:
                        print(
                            f"[{datetime.datetime.now()}] ⚠️ AVISO: API respondeu, mas houve erro: {data}"
                        )
                else:
                    print(
                        f"[{datetime.datetime.now()}] ❌ ERRO API: {response.status} - {await response.text()}"
                    )
                    return False
