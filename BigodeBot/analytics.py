"""
Analytics module for generating player and economy reports.
Provides functions to analyze player data and generate statistics.
"""
...
def export_players_csv():
    ...
    with open("exports/players_export.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["Nome", "Kills", "Deaths", "K/D", "Killstreak", "Longest Shot"]
        )
...
def generate_weekly_report():
    """Generate comprehensive analytics report for players and economy."""
    players_db = load_json("players_db.json")
    economy = load_json("economy.json")
    ...
    report = {
        ...
        "stats": {
            ...
            "avg_kills_per_player": (
                round(total_kills / total_players, 2) if total_players > 0 else 0
            ),
        },
