...
class ClanRepository(BaseRepository):
    ...
    def add_member(self, clan_id: int, discord_id: str | int, role: str = "member") -> bool:
        """
    ...
    def declare_war(self, clan1_id: int, clan2_id: int, duration_hours: int = 48) -> int | None:
        """
    ...
    def add_war_points(self, war_id: int, clan_id: int, points: int) -> bool:
        ...
        if war["clan1_id"] == clan_id:
            update_query = "UPDATE clan_wars SET clan1_points = clan1_points + ? WHERE id = ?"
        elif war["clan2_id"] == clan_id:
            update_query = "UPDATE clan_wars SET clan2_points = clan2_points + ? WHERE id = ?"
        else:
            return False
    ...
    def end_war(self, war_id: int, status: str = "finished") -> bool:
        ...
        Returns:
            True if successful, False otherwise
        """
        query = "UPDATE clan_wars SET status = ?, expires_at = datetime('now') WHERE id = ?"
