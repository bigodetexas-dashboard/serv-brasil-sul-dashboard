...
def print_header():
    ...
    # Use a safe way to clear terminal without user-provided input
    command = "cls" if os.name == "nt" else "clear"
    subprocess.run(command, shell=True)  # nosec B602 B607
    print()
    print("   " + "=" * 78)
    print(
        "   "
        + Colors.OKGREEN
        + Colors.BOLD
        + "BIGODEBOT ATIVO - MONITORANDO SERVIDOR DAYZ"
        + Colors.ENDC
    )
    print("   " + "=" * 78)
    print()
