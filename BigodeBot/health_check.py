...
def check_api_endpoints():
    ...
    endpoints = ["/api/stats", "/api/user/profile", "/api/shop/items"]
    results = {}
    for endpoint in endpoints:
        try:
            response = requests.get(f"{DASHBOARD_URL}{endpoint}", timeout=5)
            # Some endpoints might require authentication (401), which is fine for health check if it responds
...
def check_data_files():
    ...
    # SQLite Unified Database is the new source of truth
    files = ["bigode_unified.db", "config.json"]
