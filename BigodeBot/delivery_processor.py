...
class DeliveryProcessor:
    ...
    def connect_ftp(self) -> Optional[ftplib.FTP_TLS]:
        """Conecta ao servidor FTP usando TLS para segurança"""
        try:
            ftp = ftplib.FTP_TLS()
            ftp.connect(self.ftp_host, self.ftp_port, timeout=30)
            ftp.login(self.ftp_user, self.ftp_pass)
            ftp.prot_p()  # Ativa proteção de dados (encriptação)
            return ftp
        except (ftplib.all_errors, TimeoutError) as e:
            print(f"Erro ao conectar FTP (TLS): {e}")
            return None
