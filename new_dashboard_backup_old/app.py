...
def health():
    ...
    return jsonify(
        {"status": "healthy", "timestamp": str(datetime.now()), "version": "2.0"}
    )
...
def api_user_achievements():
    ...
    ach_list = result["achievements"] if result and result["achievements"] else {}

    return jsonify(
        {
            "first_kill": ach_list.get("first_kill", False),
            "survivor": ach_list.get("survivor", False),
            "rich": ach_list.get("rich", False),
            "builder": ach_list.get("builder", False),
            "hunter": ach_list.get("hunter", False),
            "explorer": ach_list.get("explorer", False),
        }
    )
...
def api_achievements_all():
    ...
    try:
        ...
        ach_list = [dict(row) for row in cur.fetchall()]
...
def api_history_add():
    ...
    try:
        ...
        return jsonify({"success": True, "event_id": result["event_id"]})
...
def api_settings_update():
    ...
    try:
        ...
        return jsonify({"success": True, "settings": dict(updated_settings)})
...
def api_banco_transactions():
    ...
    try:
        ...
        cur.execute(
            "SELECT transactions FROM economy WHERE discord_id = %s", (str(user_id),)
        )
        row = cur.fetchone()
