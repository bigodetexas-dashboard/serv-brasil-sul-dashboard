...
def create_complete_spawn(
    events_path, positions_path, item_name, x, z, quantity=1, rotation=0
):
    """
...
def clear_processed_spawns():
    ...
    if "pending_spawns" in queue:
        queue["pending_spawns"] = [
            s for s in queue["pending_spawns"] if s.get("status") != "processed"
        ]
...
def process_spawn_queue():
    """
    Processa todos os spawns pendentes (XML local)
    Returns: (bool, str)
    """
    pending = get_pending_spawns()
    if not pending:
        return False, "Nenhum spawn pendente."

    # Usamos os arquivos locais definidos no topo ou temporários
    events_xml = "events_custom.xml"
    positions_xml = "cfgeventspawns_custom.xml"

    # Limpa arquivos temporários se existirem
    if os.path.exists(events_xml):
        os.remove(events_xml)
    if os.path.exists(positions_xml):
        os.remove(positions_xml)
    ...
    for sid, data in pending.items():
        ...
        success, _ = create_complete_spawn(
            events_xml,
            positions_xml,
            data["item_name"],
            data["x"],
        ...
        if success:
            mark_spawn_processed(sid)
            success_count += 1

    return success_count > 0, f"{success_count} spawns processados em XML local."
