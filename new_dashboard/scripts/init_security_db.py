...
def init_security_db():
    ...
    try:
        ...
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_pi_xbox_id ON player_identities(xbox_id)"
        )
        # Index on Nitrado ID for fast lookup
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_pi_nitrado_id ON player_identities(nitrado_id)"
        )
        # Index on IP for fast lookup of alts
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_pi_last_ip ON player_identities(last_ip)"
        )
